...
//		}
	...
	interface Share {
		 ...
		 * The sharer's status becomes
		 * {@ link SharerStatus.PairingResponseStatus#PENDING_DISCONNECTION} until
...
//							   byte[] committedDeRecShareBytes);
	 ...
	 */
	List<? extends Share> getShares();


	/**
	 * Removes a sharer (identified by SharerStatus) as seen by this helper
	 * @param sharerStatus sharer
	 */
	void removeSharer(SharerStatus sharerStatus);

	/**
	 * Get the list of all sharers that this helper is helping
	 * @return list of sharers
	 */
	List<? extends SharerStatus> getSharers();

	/**
	 * Removes a secret stored for a given sharer.
	 * @param sharerStatus sharer
	 * @param secretId Secret id
	 */
	void removeSecret(SharerStatus sharerStatus, DeRecSecret.Id secretId);

	/**
	 * Removes a share (version number) stored for a given <sharer, secret> pair
	 * @param sharerStatus sharer
	 * @param secretId Secret id
	 * @param versionNumber Version number to be removed
	 */
	void removeVersion(SharerStatus sharerStatus, DeRecSecret.Id secretId,
					   int versionNumber);

	/**
	 * Gets all secrets stored for a given sharer
	 * @param sharerStatus sharer
	...
	/**
	 * Gets list of all versions numbers stored for a given secret
	 * @param secretId secret id
	 * @return list of all versions numbers stored for a given secret
	 */
